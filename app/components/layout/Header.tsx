...
export function Header() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'glass border-b border-white/5 shadow-2xl' : 'bg-transparent'}`}>
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 md:h-18">
          ...
          <div className="hidden lg:flex items-center gap-1">
            {NAVIGATION_LINKS.map((link) => (
              <Link key={link.href} href={link.href} className="px-4 py-2 text-slate-300 hover:text-cyan-400 text-sm font-medium transition-smooth rounded-lg hover:bg-white/4">
                {link.label}
              </Link>
            ))}
          </div>
        ...
        {mobileOpen && (
          <div className="lg:hidden pb-4 border-t border-white/5 pt-4 space-y-1">
            {NAVIGATION_LINKS.map((link) => (
              <Link key={link.href} href={link.href} onClick={() => setMobileOpen(false)} className="block px-4 py-3 text-slate-300 hover:text-cyan-400 text-sm font-medium rounded-lg hover:bg-white/4">
                {link.label}
              </Link>
            ))}
